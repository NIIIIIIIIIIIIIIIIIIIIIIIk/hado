...
const LANDINGS: LandingCard[] = [
  {
    ...
    color: '#ff6b6b',
    icon: '✦',
    description: 'абсурд-триллер',
    backgroundImage: '/image/hub/textures/nepokoy-bg.jpg',
    lockedText: ''
  },
  {
    id: 'dune',
...
export const Hub: React.FC = () => {
  ...
  const [particles, setParticles] = useState<Array<{ id: number; x: number; y: number; duration: number; size: number; delay: number }>>([]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const bgOffsetX = x * 15;
      const bgOffsetY = y * 15;
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          backgroundImage: `url(${currentCard?.backgroundImage || '/image/hub/textures/nepokoy-bg.jpg'})`,
        ...
        <div className={styles.centerWrapper}>
          <header className={styles.header}>
            <div className={styles.logoWrapper}>
              ...
              <span className={styles.logoText}>ВЫБОР СЦЕНЫ</span>
              <span className={styles.logoLine} />
            </div>
          </header>
                  ...
                  <div className="swiper-slide" key={card.id}>
                    <div
                      className={`
                        ...
                        ${card.status === 'locked' ? styles.cardLocked : ''}
                      `}
                      data-id={card.id}
                      onClick={() => handleCardClick(card)}
                      style={{
                        '--card-color': card.color,
                        transform: activeIndex === index && !isTransitioning && card.status === 'active' ? `
                          perspective(1000px)
                    ...
                    >
                      ...
                      {card.status === 'locked' && (
                        <div className={styles.lockedOverlay}>
                          ...
                          <div className={styles.lockedText}>{card.lockedText}</div>
                        </div>
                      )}
                      ...
                      <div className={styles.handwrittenLabel}>
                        <span>№ {index + 1}</span>
                      </div>
          ...
          <footer className={styles.footer}>
            <span className={styles.hint}>
              {LANDINGS[activeIndex]?.status === 'active' ? 'нажми чтобы войти' : 'доступ закрыт'}
            </span>
            <span className={styles.swipeHint}>← листай →</span>
          </footer>
        </div>
      </div>
