...
export const MainApp: React.FC = () => {
  ...
  useEffect(() => {
    const checkAccess = () => {
      const hasAccess = localStorage.getItem('calendar_access') === 'true';
  ...
  const handleTicketDownload = () => {
    // Создаём ссылку на PDF
    const link = document.createElement('a');
    link.href = '/tickets/eto_ne_troyan.pdf';
    link.download = 'eto_ne_troyan.pdf';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    setToast({ message: '🎟️ БИЛЕТ СКАЧАН', type: 'success' });
  };
  ...
  // ===== КАЛЕНДАРЬ ЗАБЛОКИРОВАН =====
  if (isLocked) {
    return (
      <div className={styles.lockedContainer}>
        <div className={styles.lockedContent}>
          <div className={styles.lockedIcon}>🔒</div>
          <h1 className={styles.lockedTitle}>ДОСТУП ЗАКРЫТ</h1>
          <p className={styles.lockedText}>Календарь находится в разработке</p>
          <div className={styles.lockedSubtext}>Скоро будет доступен</div>
          
          {/* ===== КНОПКА СКАЧАТЬ БИЛЕТ ===== */}
          <button className={styles.ticketDownloadButton} onClick={handleTicketDownload}>
            🎟️ СКАЧАТЬ БИЛЕТ
          </button>
          
          <button className={styles.lockedBackButton} onClick={handleExit}>
            ← ВЕРНУТЬСЯ В ХАБ
          </button>
          
          <button 
            className={styles.secretUnlock} 
            onClick={unlockCalendar}
            title="Секретная разблокировка"
          >
            🔓
          </button>
        </div>
        ...
        <header className={styles.appHeader}>
          ...
          <div className={styles.headerRight}>
            <div className={styles.terminalBox}>
              {/* ===== КНОПКА СКАЧАТЬ БИЛЕТ В ХЕДЕРЕ ===== */}
              <button className={styles.ticketButton} onClick={handleTicketDownload} title="Скачать билет">
                [🎟️ БИЛЕТ]
              </button>
